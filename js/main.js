$(document).ready(function(){
  // Starts the music
  soundManager.createSound({ url : 'sounds/music.mp3' }).play();
    ...
    if (currentLevel < levelParams.length) {
      ...
      $("#content").html('<a class="button" id="other-player">Player Two - you\'re up!</a>');
        ...
        if (playerOneScore > duckScore) {
          ...
          $("#content").css({"background": random_hsl_color })
          }, 100);
          $("#content").append("<h1>PLAYER ONE WINS!</h1>");
          soundManager.createSound({ url : 'sounds/airhorn.mp3', "loops":2 }).play();
...
$('#content').animate({opacity: 0}, 0).css({'background-image': 'url(\'../images/background.png\');'}).animate({opacity: 1},2300);
